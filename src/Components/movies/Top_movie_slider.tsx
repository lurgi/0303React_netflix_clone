...
export default function TopMovieSlider() {
      ...
      <AnimatePresence initial={false} custom={back1}>
        ...
        >
          {topMovies[6 * topMoiveIndex - 1] ? (
            <Box
              bgphoto={makeImg(
                popularMovies.data?.results.slice(1, 19)[6 * topMoiveIndex - 1]
                  .backdrop_path || ""
              )}
            >
              <h1>
          ...
          {topMovies[6 * topMoiveIndex + 6] ? (
            <Box
              bgphoto={makeImg(topMovies[6 * topMoiveIndex + 6].backdrop_path)}
            >
