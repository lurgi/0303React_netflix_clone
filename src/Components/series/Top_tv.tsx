...
import { getTopSeries, IgetSeries } from "../../api";
...
export default function TopTv() {
  const { data } = useQuery<IgetSeries>(["series", "topRated"], getTopSeries);
  ...
  const topSeries = data?.results.slice(1, 19) || [];
  return (
    <RowWrapper>
      <RowTitle>찬사를 받은 시리즈</RowTitle>
      ...
      <AnimatePresence initial={false} custom={back}>
        ...
        >
          {topSeries[6 * topMoiveIndex - 1] ? (
            <Box
              bgphoto={makeImg(
                data?.results.slice(1, 19)[6 * topMoiveIndex - 1]
                  .backdrop_path || ""
              )}
            >
              <h1>
